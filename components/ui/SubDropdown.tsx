    ...
    <div className={`relative inline-block w-full px-4 ${customStyle?.container || ''}`}>
      ...
      {isOpen && (
        <ul
          ...
          className={`absolute z-10 w-full bg-white border text-sm border-gray-300 rounded mt-1 shadow-lg ${customStyle?.list || ''}`}
        >
          {category.items.map((item, index) => (
            <li
              key={index}
              onClick={() => handleItemClick(item)}
              className={`px-4 py-2 hover:bg-gray-100 cursor-pointer ${customStyle?.listItem || ''}`}
            >
              {item}
            </li>
          ))}
